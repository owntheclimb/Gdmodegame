...
func _process(_delta: float) -> void:
	...
	if selection_manager and selection_manager.selected:
		...
		if selected.is_in_group("villager"):
			selected_label.text = "Selected: %s (H %.0f | Hu %.0f)" % [
				selected.name,
				selected.health,
				selected.hunger
			]
		elif selected.is_in_group("construction_site"):
			var site := selected
			selected_label.text = "Selected: Construction (%.0f%%)" % [
				(site.progress / max(site.build_time, 0.01)) * 100.0
			]
		else:
			selected_label.text = "Selected: %s" % selected.name
	else:
