...
@onready var romance_manager: RomanceManager = _get_romance_manager()
	...
	match state:
		...
		State.ROMANCE:
			...
			if population_manager:
				var partner := romance_manager.claim_partner(self) if romance_manager else null
				population_manager.queue_birth(global_position + Vector2(10, 0), self, partner)
			if romance_manager:
				romance_manager.clear_pair(self)
	state = State.IDLE
...
func start_romance(partner: Node2D) -> void:
	...
	if romance_manager and partner is Villager:
		romance_manager.register_pair(self, partner)
...
func _get_romance_manager() -> RomanceManager:
	return get_tree().get_first_node_in_group("romance_manager")
